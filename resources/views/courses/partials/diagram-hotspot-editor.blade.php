    ...
    <div class="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 overflow-hidden border border-slate-100" @click.stop>
        <div class="flex justify-between items-center pb-4 border-b border-slate-100">
            <h3 class="text-lg font-bold text-slate-800 flex items-center gap-2">
                ...
                <span>{{ __('Upload / Ganti Diagram Gambar') }}</span>
            </h3>
        ...
        <form @submit.prevent="uploadDiagram($event)" class="mt-4 space-y-4">
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5">{{ __('Pilih Lokasi Modul / Bab') }}</label>
            ...
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">{{ __('File Gambar Diagram') }}</label>
                <div class="border-2 border-dashed border-slate-200 hover:border-blue-500 rounded-xl p-6 text-center bg-slate-50/50 transition cursor-pointer relative"
                    ...
                    <div x-show="!imagePreview" class="space-y-2">
                        ...
                        <p class="text-sm font-semibold text-slate-700">{{ __('Pilih atau Seret Gambar di sini') }}</p>
                        <p class="text-xs text-slate-400">{{ __('JPG, PNG, WebP, atau SVG (Maks. 5MB)') }}</p>
                    </div>
            ...
            <div class="flex items-center justify-between pt-4 border-t border-slate-100">
                <div>
                    <button type="button" x-show="diagramObj" @click="showUploadModal = false; confirmDeleteDiagram()" class="px-3 py-2 text-xs font-bold text-red-600 hover:bg-red-50 rounded-xl transition border border-red-100 flex items-center gap-1.5">
                        ...
                        <span>{{ __('Hapus Gambar Diagram') }}</span>
                    </button>
                </div>
                <div class="flex items-center gap-2">
                    ...
                    <button type="submit" :disabled="uploading" class="px-5 py-2 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl shadow-lg shadow-blue-500/20 transition disabled:opacity-50 flex items-center gap-2">
                        <span x-show="!uploading">{{ __('Simpan Diagram') }}</span>
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 overflow-hidden border border-slate-100" @click.stop>
        <div class="flex justify-between items-center pb-4 border-b border-slate-100">
            <h3 class="text-lg font-bold text-slate-800 flex items-center gap-2">
                ...
                <span x-text="activeHotspot.id ? '{{ __('Edit Hotspot') }}' : '{{ __('Tambah Hotspot Baru') }}'"></span>
            </h3>
        ...
        <form @submit.prevent="saveHotspotForm()" class="mt-4 space-y-4">
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">{{ __('Label Hotspot') }}</label>
                <input type="text" x-model="activeHotspot.label" required placeholder="{{ __('Contoh: Titik 1 atau Katup Utama') }}"
            ...
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">{{ __('Tipe Aksi Hotspot') }}</label>
                <div class="grid grid-cols-2 gap-3">
                    <button type="button"
                        ...
                        <div class="flex items-center gap-2 font-bold text-sm">
                            ...
                            <span>{{ __('Navigasi Subbab') }}</span>
                        </div>
                        <span class="text-[11px] text-slate-500 mt-1">{{ __('Scroll langsung ke Modul/Subbab tertentu') }}</span>
                    </button>
                    ...
                    <button type="button"
                        ...
                        <div class="flex items-center gap-2 font-bold text-sm">
                            ...
                            <span>{{ __('Pop-Up Informasi') }}</span>
                        </div>
                        <span class="text-[11px] text-slate-500 mt-1">{{ __('Tampilkan modal info penjelasan singkat') }}</span>
                    </button>
                </div>
            </div>
            ...
            <div x-show="activeHotspot.action_type === 'popup'" class="space-y-3">
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">{{ __('Judul Pop-Up') }}</label>
                    <input type="text" x-model="activeHotspot.popup_title" placeholder="{{ __('Contoh: Penjelasan Pompa Utama') }}"
                           class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 text-sm font-semibold text-slate-800 outline-none transition">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">{{ __('Isi Penjelasan Pop-Up') }}</label>
                    <textarea x-model="activeHotspot.popup_content" rows="3" placeholder="{{ __('Tuliskan penjelasan rinci mengenai komponen ini...') }}"
            ...
            <div class="flex items-center justify-between pt-4 border-t border-slate-100">
                <div>
                    <button type="button" x-show="activeHotspot.id" @click="deleteHotspot(activeHotspot.id)" class="px-3 py-2 text-xs font-bold text-red-700 bg-red-50 hover:bg-red-100 border border-red-200 rounded-xl transition flex items-center gap-1.5">
                        ...
                        <span>{{ __('Hapus 1 Hotspot Ini') }}</span>
                    </button>
                </div>
                <div class="flex items-center gap-2">
                    ...
                    <button type="submit" :disabled="savingHotspot" class="px-5 py-2 text-sm font-bold text-white bg-green-600 hover:bg-green-700 rounded-xl shadow-lg shadow-green-600/20 transition disabled:opacity-50 flex items-center gap-2">
                        <span x-show="!savingHotspot">{{ __('Simpan') }}</span>
...
<div x-show="activePopupHotspot"
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 overflow-hidden border border-slate-100" @click.stop>
        ...
        <div class="mt-4 space-y-3">
            <div class="text-sm text-slate-600 leading-relaxed whitespace-pre-line font-medium" x-text="activePopupHotspot?.popup_content || '{{ __('Tidak ada deskripsi tambahan.') }}'"></div>
        </div>
