...
<x-app-layout>
    <div class="py-6 select-none">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @if (session('error'))
                <div class="rounded-xl border border-rose-100 bg-rose-50 p-4 text-sm font-semibold text-rose-700">
                    {{ session('error') }}
                </div>
            @endif
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-indigo-100 overflow-hidden">
                        ...
                        <div class="p-6 space-y-2">
                            <a href="{{ route('quizzes.index', $course) }}"
                               class="flex items-center justify-center gap-2 w-full px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-xl transition">
