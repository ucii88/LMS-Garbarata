...
<x-app-layout>
    ...
    >
        {{-- Flash Messages --}}
        @if (session('success'))
            <div class="rounded-xl border border-emerald-100 bg-emerald-50/60 p-4 text-xs font-semibold text-emerald-800 shadow-sm flex items-center gap-2">
        ...
        @if (session('error'))
            <div class="rounded-xl border border-rose-100 bg-rose-50/60 p-4 text-xs font-semibold text-rose-800 shadow-sm flex items-center gap-2">
        ...
        <section class="relative rounded-2xl overflow-hidden bg-gradient-to-r from-slate-900 via-slate-800 to-indigo-950 p-8 shadow-sm border border-slate-800">
            ...
            <div class="relative z-10 flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                <div class="space-y-2">
                    ...
                    <p class="text-xs text-slate-300 leading-relaxed">Tambah, edit, dan hapus akun pengguna sistem LMS Garbarata.</p>
                </div>
                <button
                    @click="showModal = true"
                    class="shrink-0 inline-flex items-center justify-center gap-2 rounded-xl bg-white hover:bg-slate-100 text-slate-900 px-5 py-3 text-xs font-bold transition-all shadow-sm border border-slate-200"
                >
            ...
            <form method="GET" action="{{ route('admin.users.index') }}" class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div class="relative flex-1">
                    ...
                    <input
                        ...
                        class="block w-full rounded-lg border-slate-200 pl-9 text-xs shadow-sm focus:border-blue-500 focus:ring-blue-500"
                    >
                </div>
                <select name="role" class="rounded-lg border-slate-200 text-xs shadow-sm focus:border-blue-500 focus:ring-blue-500 w-full sm:w-44">
                ...
                <button type="submit" class="inline-flex items-center justify-center rounded-lg bg-slate-900 hover:bg-slate-800 px-4 py-2 text-xs font-bold text-white transition shadow-sm w-full sm:w-auto">
                    Filter
                </button>
                @if($search || $roleFilter)
                    <a href="{{ route('admin.users.index') }}" class="inline-flex items-center justify-center rounded-lg border border-slate-200 bg-white hover:bg-slate-50 px-4 py-2 text-xs font-bold text-slate-600 transition w-full sm:w-auto">
                        Reset
                    </a>
                @endif
            </form>
        ...
        >
            <div class="relative w-full max-w-sm mx-4 rounded-2xl bg-white border border-slate-200 shadow-2xl overflow-hidden">
                <div class="p-6 text-center space-y-3">
                    ...
                    <p class="text-xs text-slate-500">Anda akan menghapus akun <span class="font-bold text-slate-700" x-text="deleteUserName"></span>. Tindakan ini tidak dapat dibatalkan.</p>
                </div>
                <div class="flex gap-3 border-t border-slate-100 bg-slate-50 px-6 py-4">
                    <button
                        type="button"
                        @click="deleteUserId = null"
                        class="flex-1 py-2 text-xs font-bold text-slate-600 border border-slate-200 rounded-xl hover:bg-slate-100 transition"
                    >
                        Batal
                    </button>
                    <form :action="`/admin/users/${deleteUserId}`" method="POST" class="flex-1">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="w-full py-2 text-xs font-bold text-white bg-rose-600 hover:bg-rose-700 rounded-xl shadow-sm transition">
                            Ya, Hapus
                        </button>
                    </form>
                </div>
            ...
            <div class="flex min-h-screen items-center justify-center px-4 py-10 text-center sm:block sm:p-0">
                ...
                >
                    <div class="border-b border-slate-100 px-6 py-5 flex items-center justify-between">
                        <div>
                            <h3 class="text-sm font-bold text-slate-900" id="add-user-modal-title">Tambah Pengguna Baru</h3>
                    ...
                    <form id="addUserForm" action="{{ route('admin.users.store') }}" method="POST" class="space-y-4 px-6 py-5 text-xs text-slate-700">
                        ...
                        <div class="space-y-1">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="Masukkan nama lengkap" class="block w-full rounded-lg border-slate-200 text-xs shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            @error('name')
                        ...
                        <div class="space-y-1">
                            ...
                            <input type="email" name="email" id="email" value="{{ old('email') }}" required placeholder="nama@contoh.com" class="block w-full rounded-lg border-slate-200 text-xs shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            @error('email')
                                <p class="text-[10px] font-semibold text-rose-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="space-y-1">
                            ...
                            <input type="password" name="password" id="password" required placeholder="Minimal 8 karakter" class="block w-full rounded-lg border-slate-200 text-xs shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            @error('password')
                                <p class="text-[10px] font-semibold text-rose-600">{{ $message }}</p>
                            @enderror
                        </div>
