@section('topbar_title', __('Edit Pengguna'))

<x-app-layout>
    <div class="max-w-2xl mx-auto space-y-6">
        {{-- Breadcrumb --}}
        <nav class="flex items-center gap-2 text-sm text-slate-400" aria-label="Breadcrumb">
            <a href="{{ route('admin.users.index') }}" class="font-semibold hover:text-slate-600 transition">{{ __('Kelola Pengguna') }}</a>
        ...
        <section class="relative rounded-2xl overflow-hidden bg-gradient-to-r from-slate-900 via-slate-800 to-indigo-950 p-7 shadow-sm border border-slate-800">
            ...
            <div class="relative z-10 flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-slate-400 mt-0.5">{{ $user->email }} · {{ __('Terdaftar') }} {{ $user->created_at->format('d M Y') }}</p>
                </div>
            </div>
        </section>
        ...
        <section class="rounded-2xl border border-[#f0f0f0] bg-white shadow-sm overflow-hidden">
            <div class="border-b border-[#f0f0f0] px-6 py-4">
                <h2 class="text-base font-bold text-slate-800">{{ __('Informasi Pengguna') }}</h2>
                <p class="text-[10px] text-slate-400 mt-0.5">{{ __('Perbarui nama, email, dan peran pengguna.') }}</p>
            </div>
            ...
            <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="px-6 py-5 space-y-5 text-sm text-slate-700">
                ...
                <div class="space-y-1.5">
                    <label for="edit_name" class="block text-sm font-bold text-slate-700">
                        {{ __('Nama Lengkap') }}
                        <span class="font-medium text-slate-400">{{ __('(opsional)') }}</span>
                    </label>
                    <input
                        type="text"
                        name="name"
                ...
                {{-- Email --}}
                <div class="space-y-1.5">
                    <label for="edit_email" class="block text-sm font-bold text-slate-700">{{ __('Alamat Email') }}</label>
                    <input
                        type="email"
                        name="email"
                        id="edit_email"
                ...
                {{-- Role --}}
                <div class="space-y-1.5">
                    <label for="edit_role" class="block text-sm font-bold text-slate-700">{{ __('Peran') }}</label>
                    <select
                        name="role"
                        id="edit_role"
                        required
                    ...
                    >
                        <option value="peserta"    @selected(old('role', $user->role) === 'peserta')>{{ __('Peserta') }}</option>
                        <option value="instruktur" @selected(old('role', $user->role) === 'instruktur')>{{ __('Instruktur') }}</option>
                ...
                <div class="border-t border-[#f0f0f0] pt-5 space-y-1">
                    <h3 class="text-sm font-bold text-slate-800">{{ __('Ganti Password') }}</h3>
                    <p class="text-[10px] text-slate-400">{{ __('Kosongkan jika tidak ingin mengubah password.') }}</p>
                </div>

                {{-- New Password --}}
                <div class="space-y-1.5" x-data="{ showPwd: false }">
                    <label for="edit_password" class="block text-sm font-bold text-slate-700">{{ __('Password Baru') }}</label>
                    <div class="relative">
                        <input
                ...
                <div class="space-y-1.5" x-data="{ showConfirm: false }">
                    <label for="edit_password_confirmation" class="block text-sm font-bold text-slate-700">{{ __('Konfirmasi Password Baru') }}</label>
                    <div class="relative">
                        <input
