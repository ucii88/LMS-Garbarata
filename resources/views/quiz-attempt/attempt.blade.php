...
<div id="connection-banner" class="hidden fixed top-0 left-0 right-0 z-50 px-4 py-2.5 text-center text-sm font-bold transition-all duration-300">
                ...
                <div class="question-card bg-white border border-slate-100 rounded-2xl shadow-sm p-6 {{ $loop->iteration > 1 ? 'hidden' : '' }}"
                    ...
                    <div class="flex items-start justify-between gap-3 mb-4">
                        <div class="flex items-center gap-2">
                            <span class="w-8 h-8 rounded-full bg-blue-600 text-white text-base font-bold flex items-center justify-center shrink-0">
                                {{ $loop->iteration }}
                            </span>
                    ...
                    {{-- Teks Soal --}}
                    <p class="text-base font-medium text-slate-800 leading-relaxed mb-1">
                        ...
                        @if($question->type === 'multiple_choice')
                            @foreach($question->options as $j => $option)
                                <label class="flex items-start gap-3 p-3.5 border-2 border-slate-100 rounded-xl cursor-pointer transition hover:border-blue-300 has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50">
                                    ...
                                    <span class="text-base text-slate-700 leading-relaxed">
                            ...
                            <div class="space-y-4">
                                ...
                                @foreach($question->options as $option)
                                    <div class="flex flex-col gap-2 p-3 bg-slate-50/50 rounded-xl border border-slate-100 shadow-sm">
                                        <div class="text-sm font-bold text-slate-700 bg-slate-100/80 px-2.5 py-1.5 rounded-lg border border-slate-200/50">
                                            {{ $option->option_text }}
                                        </div>
                    ...
                    <div class="grid grid-cols-5 lg:grid-cols-4 gap-1.5" id="nav-grid">
                        @foreach($questions as $i => $question)
                            <button type="button" onclick="jumpTo({{ $loop->iteration }})"
                                    ...
                                    class="nav-btn w-8 h-8 text-sm font-bold rounded-lg border-2 border-slate-200 text-slate-500 hover:border-blue-400 transition">
                                {{ $loop->iteration }}
                            </button>
                        @endforeach
                    </div>
