<x-guest-layout>
    ...
    <div class="text-center mb-2">
        ...
        <p class="text-xs text-slate-500">Masukkan email & kata sandi untuk masuk ke kelas LMS</p>
    </div>

    <!-- Session Status -->
    <x-auth-session-status class="mb-4 text-xs font-semibold text-emerald-600 bg-emerald-50 border border-emerald-200 px-3 py-2.5 rounded-xl" :status="session('status')" />
    ...
    <form method="POST" action="{{ route('login') }}" class="flex flex-col gap-4">
        ...
        <div>
            ...
            <div class="relative">
                <input id="email" 
                       class="w-full bg-slate-50/50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-950 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200" 
                       type="email" 
                       name="email" 
                       value="{{ old('email') }}" 
        ...
        <div class="flex items-center justify-between mt-1">
            <label for="remember_me" class="inline-flex items-center cursor-pointer select-none">
                <input id="remember_me" 
                       ...
                       name="remember">
                <span class="ms-2.5 text-xs text-slate-500 hover:text-slate-700 transition duration-200">Ingat saya di perangkat ini</span>
            </label>
        </div>
        ...
        <div class="mt-2">
            <button type="submit" 
                    class="w-full py-3 px-5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white text-sm font-semibold shadow-md hover:shadow-lg transition duration-300 flex items-center justify-center gap-2">
